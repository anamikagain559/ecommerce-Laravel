...
class ProductController extends Controller
{
    ...
    {
        $products = DB::table('products')->where('category_id', $id)->paginate(30);
        $brands = DB::table('products')->where('id', $id)->select('brand_id')->groupBy('brand_id')->get();
        return view('pages.all_products', compact('products', 'brands'));
    }
    public function productByBrand($id)
    {
        $products = DB::table('products')->where('brand_id', $id)->get(10);
        $brands = DB::table('products')->where('id', $id)->select('brand_id')->groupBy('brand_id')->get();
        return view('pages.index2', compact('products', 'brands'));
    }
}
